...
list_score_cat = [
    ("high" if s > 2200 else "medium" if s > 2000 else "low")
    for s in scores.values()
]
dict_score_cat = {
    cat: list_score_cat.count(cat) for cat in ["high", "medium", "low"]
}
print(f"Score categories: {dict_score_cat}")
achievements_count = {
    name: len(achs[name]) for name, is_active in p_is_act.items() if is_active
}
...
unique_achievements = {
    achievement
    for achievements in achs.values()
...
print(
    f"Top performer: {top_performer} ({scores['alice']} points, "
